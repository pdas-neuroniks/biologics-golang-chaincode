...
// OrderContract defines the smart contract structure
...
func (c *OrderContract) CreateOrder(ctx contractapi.TransactionContextInterface, inputData string) (*Order, error) {
	var orderData Order
...
func (c *OrderContract) UpdateOrderStatus(ctx contractapi.TransactionContextInterface, statusUpdateData string) (*Order, error) {
...
func (c *OrderContract) GetOrder(ctx contractapi.TransactionContextInterface, orderID string) (*Order, error) {
	orderBytes, err := ctx.GetStub().GetState(orderID)
	if err != nil {
		return nil, fmt.Errorf("failed to read from world state: %v", err)
	}
...
func (c *OrderContract) GetOrderHistory(ctx contractapi.TransactionContextInterface, orderID string) ([]HistoryQueryResult, error) {
...
func (c *OrderContract) GetAllOrdersWithPagination(ctx contractapi.TransactionContextInterface, pageSizeStr string, bookmark, sortField, sortOrder string) (*PaginatedQueryResponse, error) {
	// 1. Parse pageSize
...
func order() {
	...
	if err != nil {
		fmt.Printf("Error creating order contract chaincode: %v\n", err)
		return
	}

	if err := chaincode.Start(); err != nil {
		fmt.Printf("Error starting order contract chaincode: %v\n", err)
	}
}
